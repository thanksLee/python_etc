...
```
- enumerate
    > 순서가 있는 자료형(리스트, 튜플, 문자열)을 입력받아 인덱스 값을 포함하는
      객체를 리턴하는 메소드이다.
```
### Numpy
```
...
- 빠르고 메모리를 효율적으로 사용하고, 벡터 산술연산과 브로드캐스팅 기능을
  제공하므로써 다차원 배열을 제공한다.
- 반복문을 작성할 필요 없이 전체 데이터 배열에 대해서 빠른 연산을 제공한다.
- 배열 데이터를 디스크에 쓰거나, 읽을 수 있는 기능, 메모리에 올려진 파일을 사용
- C, C++, 포트란 쓰여진 코드를 통합
- Numpy 그 자체로는 고수준의 데이터 분석 기능을 제공하지 않기 때문에 pandas를 함께
  사용하면 효율적인 작업을 할 수있다.
  Numpy + Pandas 활용을 많이 사용한다.
- Numpy 주요 기능
...
- 1차원 배열 : 벡터 - vector
- 2차원 배열 : 행렬 - mattrix
- 3차원 배열 : 3차원 이상의 배열은 다차원 배열이라 한다.
```
### Pandas
```
- ex : ex26.py, ex27.py
- 고수준의 자료구조와 파이썬을 통해서 데이터 분석을 쉽고 빠르게 분석하는 도구
- Numpy 기반에서 개발되어 Numpy 사용하는 Application에서 많이 사용되는 Library
- 자료구조
    > Serise, DataFrame 는 일반적으로 많이 사용하는 자료구조
    > Serise:
      * 일련의 객체를 담을 수 있는 1차원 배열 같은 자료구조
        (인덱스 값과 Value를 같이 리턴)
    > DataFrame:
      * 스프레드시트 형식의 자료구조로 여러개의 컬럼에 다양한 값을 담을 수 있는 자료구조
```